...
export function ImageCompressor({ file, onClear }: Props) {
  ...
  const run = React.useCallback(
    async (f: File, q: number, fmt: ImageTargetFormat, md: number) => {
  ...
  // Auto-run on mount and whenever settings change.
  React.useEffect(() => {
    run(file, quality, format, maxDim);
  }, [file, quality, format, maxDim, run]);
